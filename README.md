...
![Vista del Feed Principal de Pvox](doc/feed.png)
*Vista de usuario mostrando el feed asíncrono y la interfaz principal.*
...
* **Pipeline Multimedia en el Cliente:** 
  * Conversión automática de imágenes a formato optimizado WebP utilizando la API de Canvas en JavaScript antes del envío.
  * Procesamiento de video mediante Web Workers ejecutando FFmpeg (alojado en Netlify) para transcodificar medios pesados a 720p localmente, evitando saturar el ancho de banda del servidor.
...
![Modal de Reporte de Bugs Vacio](doc/reporte-bug-vacio.png)

![Modal de Reporte de Bugs Lleno](doc/reporte-bug-lleno.png)
*Interfaz de reporte de errores disponible para los usuarios.*
...
### 3. Entorno de Creacion y Publicacion: Pvox Studio

Para separar la experiencia de consumo de contenido de la de creacion, el sistema cuenta con **Pvox Studio**. Es un modulo dedicado que opera bajo el mismo dominio principal, ofreciendo una interfaz con un diseno avanzado y libre de distracciones exclusivo para la estructuracion de publicaciones.

* **Acceso Universal:** Disponible para todos los usuarios registrados en la plataforma, funcionando como el centro de creacion principal.
* **Gestion de Contenido:** Actua como el punto de entrada para redactar texto y gestionar la seleccion de archivos multimedia (fotos y videos).
* **Integracion del Pipeline Multimedia:** Al adjuntar archivos en Pvox Studio, el sistema activa de forma transparente la compresion a WebP y la transcodificacion de video local (via FFmpeg Web Worker) antes de comunicarse con la API de creacion.

![Interfaz de Pvox Studio](doc/pvox-studio.png)
*Entorno dedicado para la creacion y optimizacion de contenido multimedia.*

### 4. Navegacion y Visualizacion de Publicaciones

La plataforma divide el consumo de contenido en dos niveles de profundidad para optimizar el rendimiento del feed y mejorar la experiencia de usuario:

* **Vista Dedicada (Single Post View):** Al seleccionar una publicacion especifica desde el feed principal, el sistema abre una nueva ventana que renderiza la publicacion a pantalla completa, permitiendo un enfoque total en ese contenido especifico.
* **Interacciones Globales:** Tanto desde el feed asincrono como desde la vista dedicada, los usuarios tienen acceso rapido a las acciones clave: reaccionar a la publicacion (con soporte universal para cualquier emoji del teclado), guardar en favoritos, seguir al creador y emitir reportes de moderacion.
* **Motor de Comentarios Aislado:** Para mantener la carga inicial del feed ligera y la interfaz limpia, el sistema de lectura y publicacion de comentarios se habilita de manera exclusiva dentro de la pagina dedicada de cada publicacion.

![Vista Dedicada del Post](doc/vista-post.png)
*Vista a pantalla completa de una publicacion mostrando el hilo de comentarios exclusivo.*
...
![Pantalla de Mantenimiento](doc/mantenimiento.png)
*Sistema bloqueado en modo mantenimiento con campo de acceso mediante Bypass Code.*
...
![Modal de Actualización de Políticas](doc/politicas.png)
*Interrupción de sesión forzando la aceptación de nuevas políticas.*
...
### 2. Monitoreo mediante Server-Sent Events (SSE)
